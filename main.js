...
async function captureNoonImages(win, products) {
  ...
  try {
    ...
    for (let i = 0; i < products.length; i++) {
      const p = products[i];
      if (!p.image || !p.image.startsWith('http')) { products[i].image = ''; continue; }
      try {
        // ── جوهر الإصلاح ──────────────────────────────────────────────
        // لا نعتمد على thumbnail صغير ولا نصوّر صورة غير محقّقة الحجم.
        // 1) نبني مرشّحين عالي الدقة برفع مقاس نون CDN (.../NN/_/...) لقيم كبيرة.
        // 2) نحمّل كلاً ونقيس naturalWidth الفعلي — نختار الأكبر، ونتوقّف عند >=500.
        // 3) إن لم نجد صورة >=300px فعلية → نتركها بلا صورة (لا بيضاء/مدمجة/ممطوطة).
        // 4) نعرض المختارة بحجمها الحقيقي ونصوّرها (لا تمطيط 600 ثابت).
        const prep = await win.webContents.executeJavaScript(`
          (async () => {
            try {
              const orig = ${JSON.stringify(p.image)};
              // رفع مقاس رابط نون CDN: استبدل /NN/_/ بمقاسات كبيرة (يغطي 45/80/120/240...)
              ...
              cands.push(orig); // الأصلي كملاذ أخير

              const old = document.getElementById('__nashir_capture__');
              if (old) old.remove();

              // قِس الحجم الفعلي لكل مرشّح، واختر الأكبر (وتوقّف مبكراً عند صورة كبيرة)
              const measure = (u) => new Promise((res) => {
                ...
                im.onload  = () => fin(im.naturalWidth > 10 ? { w: im.naturalWidth, h: im.naturalHeight } : null);
                im.onerror = () => fin(null);
                im.src = u;
                setTimeout(() => fin(im.complete && im.naturalWidth > 10 ? { w: im.naturalWidth, h: im.naturalHeight } : null), 6000);
              });
              let chosenUrl = '', chosenW = 0, chosenH = 0;
              for (const u of cands) {
                const r = await measure(u);
                if (r && r.w > chosenW) { chosenUrl = u; chosenW = r.w; chosenH = r.h; }
                if (chosenW >= 500) break; // وجدنا صورة كبيرة — يكفي
              }
              // تجاهل الصور الصغيرة تماماً (thumbnail) — لا نصوّر شيئاً أصغر من 300px
              if (!chosenUrl || chosenW < 300 || chosenH < 300) return { ok: false, w: chosenW, h: chosenH };

              // اعرض المختارة بحجمها الحقيقي (بحد أقصى 800) داخل طبقة مربّعة بيضاء
              const box = Math.min(800, Math.max(chosenW, chosenH));
              const overlay = document.createElement('div');
              overlay.id = '__nashir_capture__';
              overlay.style.cssText = 'position:fixed;top:0;left:0;width:' + box + 'px;height:' + box + 'px;z-index:2147483647;background:#fff;display:flex;align-items:center;justify-content:center;';
              const im2 = document.createElement('img');
              im2.referrerPolicy = 'no-referrer';
              im2.style.cssText = 'max-width:' + box + 'px;max-height:' + box + 'px;object-fit:contain;display:block;background:#fff;';
              overlay.appendChild(im2);
              document.body.appendChild(overlay);
              const drawn = await new Promise((res) => {
                let done = false; const fin = (ok) => { if (done) return; done = true; res(ok); };
                im2.onload  = () => fin(im2.naturalWidth > 10);
                im2.onerror = () => fin(false);
                im2.src = chosenUrl;
                setTimeout(() => fin(im2.complete && im2.naturalWidth > 10), 6000);
              });
              if (!drawn) { overlay.remove(); return { ok: false }; }
              return { ok: true, box: box, w: chosenW, h: chosenH };
            } catch(e) { return { ok: false, err: String(e).slice(0, 80) }; }
          })()
        `).catch(() => ({ ok: false }));

        if (!prep || !prep.ok) {
          await win.webContents.executeJavaScript(`(()=>{const o=document.getElementById('__nashir_capture__');if(o)o.remove();return 1;})()`).catch(()=>{});
          products[i].image = ''; // صورة صغيرة/فاشلة — لا نرفق صورة بيضاء
          continue;
        }
        await new Promise(r => setTimeout(r, 180)); // استقرار الرسم
        ...
        // نظّف الطبقة فوراً بعد اللقطة (يمنع تداخل صور المنتجات)
        await win.webContents.executeJavaScript(`(()=>{const o=document.getElementById('__nashir_capture__');if(o)o.remove();return 1;})()`).catch(()=>{});
        ...
        try { await win.webContents.executeJavaScript(`(()=>{const o=document.getElementById('__nashir_capture__');if(o)o.remove();return 1;})()`).catch(()=>{}); } catch(e2){}
        products[i].image = '';
      }
    }
    return products;

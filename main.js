...
ipcMain.handle('fetch-bestsellers', async (_, { source, query }) => {
  const STORES = {
      ...
      extract: `
        (() => {
          ...
          for (const a of links) {
            let href = a.href || '';
            // قبول روابط المنتجات فقط (تحوي /p أو معرّف N متبوع بـ/p)
            ...
            if (!title) { const t = a.querySelector('[title]'); if (t) title = t.getAttribute('title').trim(); }
    ...
    aliexpress: {
      url: q => `https://www.aliexpress.com/w/wholesale-${encodeURIComponent(q.replace(/\s+/g,'-'))}.html`,
      extract: `
        (() => {
          const out = [];

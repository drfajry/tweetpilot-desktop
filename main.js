...
async function captureNoonImages(win, products) {
  ...
  try {
    ...
    for (let i = 0; i < products.length; i++) {
      ...
      try {
        // جهّز الصورة المكبّرة وانتظر تحميلها الفعلي (onload) لا مؤقتاً ثابتاً — يمنع اللقطة البيضاء
        const ready = await win.webContents.executeJavaScript(`
          (async () => {
            try {
              const a = Array.from(document.querySelectorAll('a[href*="/p/"]')).find(x => x.href.split('?')[0] === ${JSON.stringify(p.url)});
              const srcEl = a ? a.querySelector('img') : null;
              if (!srcEl || !srcEl.src) return false;
              // أزل أي طبقة سابقة (يمنع ظهور صورة منتج آخر)
              const old = document.getElementById('__nashir_capture__');
              if (old) old.remove();
              const overlay = document.createElement('div');
              overlay.id = '__nashir_capture__';
              overlay.style.cssText = 'position:fixed;top:0;left:0;width:600px;height:600px;z-index:2147483647;background:#fff;';
              const big = document.createElement('img');
              big.style.cssText = 'width:600px;height:600px;object-fit:contain;display:block;background:#fff;';
              big.referrerPolicy = 'no-referrer';
              overlay.appendChild(big);
              document.body.appendChild(overlay);
              // جرّب مقاساً أكبر، واسقط للأصلي عند الفشل، وانتظر التحميل الفعلي
              let hiRes = srcEl.src;
              try { hiRes = hiRes.replace(/\\/(\\d{2,3})\\/_?\\//, '/800/_/'); } catch(e){}
              const loaded = await new Promise((res) => {
                let settled = false;
                const finish = (ok) => { if (settled) return; settled = true; res(ok); };
                big.onload = () => { if (big.naturalWidth > 10) finish(true); else finish(false); };
                big.onerror = () => {
                  // اسقط للرابط الأصلي
                  big.onload = () => { if (big.naturalWidth > 10) finish(true); else finish(false); };
                  big.onerror = () => finish(false);
                  big.src = srcEl.src;
                };
                big.src = hiRes;
                setTimeout(() => finish(big.complete && big.naturalWidth > 10), 6000);
              });
              return loaded;
        ...
        `).catch(()=>false);
        if (!ready) {
          // نظّف وتجاوز هذه الصورة (تبقى بلا صورة بدل صورة بيضاء/خاطئة)
          await win.webContents.executeJavaScript(`(()=>{const o=document.getElementById('__nashir_capture__');if(o)o.remove();return 1;})()`).catch(()=>{});
          products[i].image = ''; // لا نرفق صورة بيضاء
          continue;
        }
        await new Promise(r => setTimeout(r, 200)); // استقرار الرسم

        const result = await dbg.sendCommand('Page.captureScreenshot', {
          format: 'jpeg',
          quality: 95,
          clip: { x: 0, y: 0, width: 600, height: 600, scale: 2 },
          captureBeyondViewport: true,
        });
        // نظّف الطبقة فوراً بعد اللقطة (يمنع تداخل الصور)
        await win.webContents.executeJavaScript(`(()=>{const o=document.getElementById('__nashir_capture__');if(o)o.remove();return 1;})()`).catch(()=>{});

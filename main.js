    ...
    noon: {
      ...
      extract: `
        (() => {
          const out = [];
          const seen = new Set();
          const BAD = /^(placeholder|image|loading|img|noon|null|undefined)$/i;
          const links = document.querySelectorAll('a[href*="/p/"], a[href*="/N"]');
          for (const a of links) {
            ...
            const container = a.closest('div[class*="productContainer"], div[class*="product"], li, article') || a.parentElement?.parentElement || a;

            // اسم المنتج في نون: عنصر بخاصية title أو وسم عنوان مخصص — لا النص الأطول
            let title = '';
            // 1) أي عنصر فيه خاصية title حقيقية
            const titled = container.querySelector('[title]:not([title=""])');
            if (titled) { const tv = titled.getAttribute('title').trim(); if (tv && !BAD.test(tv) && !/خصم|باقي|المخزون|وحدات/.test(tv)) title = tv; }
            // 2) alt الصورة إن كان وصفياً (يحوي حروفاً عربية أو إنجليزية كثيرة وليس أرقام/مخزون)
            if (!title) {
              const img = a.querySelector('img') || container.querySelector('img');
              if (img && img.alt) { const al = img.alt.trim(); if (al && !BAD.test(al) && !/خصم|باقي|المخزون|وحدات|^\\d/.test(al) && al.length >= 6) title = al; }
            }
            // 3) عناصر عنوان شائعة في نون
            if (!title) {
              const h = container.querySelector('[data-qa*="title"], [class*="name"], [class*="Name"], [class*="title"], [class*="Title"]');
              if (h) { const hv = (h.textContent||'').trim(); if (hv && !/خصم|باقي|المخزون|وحدات/.test(hv) && hv.length >= 6) title = hv; }
            }
            if (!title || BAD.test(title) || title.length < 6) continue;
            // رفض النص الذي يبدو سعراً/مخزوناً لا اسماً
            if (/خصم|باقي|المخزون|وحدات/.test(title) && !/[A-Za-z\\u0621-\\u064A]{4,}/.test(title.replace(/خصم|باقي|المخزون|وحدات/g,''))) continue;

            const img = a.querySelector('img') || container.querySelector('img');
            let price = '';
            const pm = container.textContent.match(/[\\d,\\.]+\\s*(?:ريال|SAR|SR)/);
            if (pm) price = pm[0];
            out.push({ url: clean, title: title.substring(0,80), price, image: img ? (img.src||img.getAttribute('data-src')||'') : '' });
            if (out.length >= 8) break;
          }
          return out;
        })()

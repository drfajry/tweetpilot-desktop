      ...
      response.on('end', () => {
        ...
        try {
          ...
          for (let i = 0; i < titleMatches.length && results.length < 8; i++) {
            ...
            // ارفض صفحات البحث/التصنيفات الواضحة فقط — اقبل الباقي
            ...
            results.push({
              ...
              isProduct: isProductPage(url), // صفحة منتج مباشرة؟
            });
          }

          // رتّب: صفحات المنتجات المباشرة أولاً
          results.sort((a, b) => (b.isProduct ? 1 : 0) - (a.isProduct ? 1 : 0));

          if (results.length === 0) {
            resolve({ success: false, error: 'لم نجد نتائج — جرّب كلمات أدق (اسم المنتج + الماركة)', products: [] });
            return;
          }
...
ipcMain.handle('fetch-bestsellers', async (_, { source, query }) => {
  ...
  // إعادة محاولة واحدة عند الحظر المؤقت

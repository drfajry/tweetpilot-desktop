      ...
      try {
        ...
        const result = await win.webContents.executeJavaScript(`
          (async () => {
            ...
            // الطريقة المجرّبة البسيطة: تركيز ثم insertText
            box.focus();
            await wait(300);
            document.execCommand('insertText', false, ${JSON.stringify(content)});
            await wait(900);

            // تشخيص: سجّل ما حدث فعلاً في المحرر (يظهر في الكونسول / سجل التطبيق)
            try {
              const _ed = box.getAttribute('contenteditable')==='true' ? box : (box.querySelector('[contenteditable="true"]')||box);
              const _btn = document.querySelector('[data-testid="tweetButton"],[data-testid="tweetButtonInline"]');
              console.log('NASHIR_DIAG', JSON.stringify({
                wantLen: ${JSON.stringify(content)}.length,
                gotLen: (_ed.textContent||'').length,
                sample: (_ed.textContent||'').substring(0,50),
                btnDisabled: _btn ? _btn.getAttribute('aria-disabled') : 'no-btn'
              }));
            } catch(e) {}
            ...
            try {
              const sent = await win.webContents.executeJavaScript(`
                (() => {
                  ...
                  if (/login|flow|access/.test(p)) return 'WAIT'; // ما زال في تسجيل الدخول
                  const box = document.querySelector('[data-testid="tweetTextarea_0"]');
                  const onCompose = p.startsWith('/compose');

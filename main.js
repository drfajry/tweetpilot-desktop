...
async function postWithPuppeteer(content) {
  return new Promise((resolve) => {
    ...
    win.webContents.on('did-finish-load', async () => {
      if (handled) return; // تجنّب التكرار عند إعادة تحميل الصفحة
      try {
        const url = win.webContents.getURL();
        // لو لا يزال في صفحة تسجيل الدخول، انتظر
        if (/\/(login|i\/flow\/login|account\/access)/.test(url)) {
          if (!win.isVisible()) { clearTimeout(timer); win.show(); }
          return;
        }
        ...
        if (!isLoggedIn) {
          // يحتاج تسجيل دخول — أظهر النافذة وانتظر (سيُعاد did-finish-load بعد الدخول)
          if (!win.isVisible()) { clearTimeout(timer); win.show(); }
          return;
        }

        // مسجّل دخول — انشر مرة واحدة فقط
        handled = true;
        clearTimeout(timer);
        win.hide();
        ...
        // لا نُنهي عند خطأ مؤقت أثناء التنقل
      }
    });

    // مهلة قصوى لتسجيل الدخول
    setTimeout(() => { if (!handled) finish({ success: false, error: 'LOGIN_TIMEOUT' }); }, 150000);

    win.webContents.on('did-fail-load', () => {
      if (!handled) { clearTimeout(timer); finish({ success: false, error: 'فشل تحميل إكس — تحقق من الإنترنت' }); }
    });
  });
}
  ...
  try {
    const result = await win.webContents.executeJavaScript(`
      (async () => {
        ...
        // انتظر حتى يصبح زر النشر مفعّلاً (حتى 6 ثوان)
        let postBtn = null;
        for(let i=0; i<12; i++){
          postBtn = document.querySelector('[data-testid="tweetButton"], [data-testid="tweetButtonInline"]');
          if(postBtn && postBtn.getAttribute('aria-disabled') !== 'true') break;
          await wait(500);
        }
        ...
        // تحقق من النشر: اختفاء صندوق الكتابة = نُشرت
    ...
    if (result === 'NO_BOX') return { success: false, error: 'تعذر فتح صندوق الكتابة — حدّث الصفحة وحاول' };
    if (result === 'NO_BTN') return { success: false, error: 'تعذر العثور على زر النشر' };
    if (result === 'DISABLED') return { success: false, error: 'زر النشر لم يُفعّل — تحقق أن التغريدة لا تتجاوز ٢٨٠ حرفاً' };
    return { success: false, error: 'نتيجة غير متوقعة: ' + result };
  } catch(e) {
...
function searchDuckDuckGo(query) {
  return new Promise((resolve) => {
    ...
    // POST هو الطريقة الأكثر قبولاً في html.duckduckgo.com
    const postData = `q=${encodeURIComponent(query)}&kl=xa-ar&df=`;
    ...
    request.setHeader('Origin', 'https://html.duckduckgo.com');
    request.setHeader('Referer', 'https://html.duckduckgo.com/');
    ...
    request.on('response', (response) => {
      ...
      response.on('end', () => {
        ...
        clearTimeout(timer);
        try {
          if (response.statusCode === 202 || response.statusCode === 429 || response.statusCode === 403) {
            resolve({ success: false, error: `محرك البحث مشغول مؤقتاً — انتظر ١٠ ثوانٍ وأعد المحاولة`, products: [], rateLimited: true });
            return;
          }
          if (response.statusCode < 200 || response.statusCode >= 300) {
            resolve({ success: false, error: `تعذر البحث (${response.statusCode})`, products: [] });
            return;
          }
          ...
          for (let i = 0; i < titleMatches.length && results.length < 8; i++) {
            ...
            // تخطّ صفحات البحث والتصنيفات
            if (isBadPage(url) || !isProductPage(url)) continue;
          ...
          if (results.length === 0) {
            resolve({ success: false, error: 'لم نجد صفحة منتج مباشرة — جرّب كلمات أدق (اسم المنتج + الماركة)', products: [] });
            return;
          }
...
ipcMain.handle('fetch-bestsellers', async (_, { source, query }) => {
  ...
  // إعادة المحاولة مع تأخير متزايد عند الحظر المؤقت

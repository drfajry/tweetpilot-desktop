...
async function captureNoonImages(win, products) {
  ...
  try {
    ...
    const captureLog = []; // سجل تشخيصي لكل منتج (يُكتب لملف بعد الانتهاء)

    for (let i = 0; i < products.length; i++) {
      const p = products[i];
      if (!p.url) { products[i].image = ''; continue; }
      try {
        // ── منطق التحقق قبل التصوير (حسب المواصفات) ───────────────────────
        // نصوّر عنصر img الرئيسي نفسه فقط — لا حاوية/بطاقة/سلايدر.
        // شروط الجاهزية: complete === true && naturalWidth>300 && naturalHeight>300 && ظاهر.
        // إن لم تتحقق: ننتظر 500ms ونعيد الفحص حتى 10 مرات. إن بقيت غير جاهزة → لا نحفظ صورة.
        // ملاحظة تقنية: في صفحة بحث نون العنصر معروض بحجم thumbnail (~150px)، فاللقطة تأخذ
        // الحجم المعروض لا naturalWidth. لذا نرفع مصدر نفس العنصر لدقة عالية (CDN /NN/_/ → /1200/_/)
        // ثم نعرضه بحجمه الحقيقي ونصوّره — يبقى "عنصر img الرئيسي" نفسه دون أي حاوية.
        const prep = await win.webContents.executeJavaScript(`
          (async () => {
            const sleep = (ms) => new Promise(r => setTimeout(r, ms));
            try {
              const url = ${JSON.stringify(p.url)};
              // عنصر الصورة الرئيسي داخل بطاقة هذا المنتج تحديداً
              const a = Array.from(document.querySelectorAll('a[href*="/p/"]')).find(x => (x.href||'').split('?')[0] === url);
              const img = a ? a.querySelector('img') : null;
              if (!img) return { ok:false, reason:'no_img', log:{ reason:'no_img' } };

              // ارفع مصدر نفس العنصر لدقة عالية (لا نُنشئ صورة منفصلة)
              const cur = img.currentSrc || img.src || '';
              const hi = cur.replace(/\\/(\\d{2,3})\\/_?\\//, '/1200/_/');
              if (hi && hi !== img.src) { try { img.src = hi; } catch(e){} }

              img.scrollIntoView({ block:'center', inline:'center' });

              // حلقة الجاهزية: حتى 10 محاولات × 500ms
              let ready = false, log = null;
              for (let t = 0; t < 10; t++) {
                ...
                log = {
                  ...
                  inView: inView,
                  currentSrc: (img.currentSrc || '').slice(0, 140)
                };
                if (img.complete === true && img.naturalWidth > 300 && img.naturalHeight > 300 && inView) { ready = true; break; }
                await sleep(500);
              }

              if (!ready) return { ok:false, reason:'not_ready', log: log };

              // اعرض العنصر نفسه بحجمه الحقيقي (بحد أقصى 800) وثبّته أعلى الصفحة للتصوير النظيف
              const box = Math.min(800, Math.max(img.naturalWidth, img.naturalHeight));
              img.dataset.nashirPrevStyle = img.getAttribute('style') || '';
              ...
              img.id = img.id || '__nashir_main_img__';
              await sleep(150); // استقرار الرسم
              return { ok:true, box: box, imgId: img.id, log: log };
            } catch(e) { return { ok:false, reason:'js_err', log:{ reason:'js_err', err:String(e).slice(0,100) } }; }
          })()
        `).catch((e) => ({ ok:false, reason:'exec_err', log:{ reason:'exec_err', err:String(e).slice(0,100) } }));

        // سجّل التشخيص قبل أي تصوير
        const logEntry = Object.assign({ url: p.url, ok: !!(prep && prep.ok), reason: prep && prep.reason }, (prep && prep.log) || {});
        captureLog.push(logEntry);
        console.log('[noon-capture]', JSON.stringify(logEntry));

        // شرط صريح: إن لم تكتمل الشروط أو naturalWidth<=300 → لا تصوير إطلاقاً، انتقل للتالي
        if (!prep || !prep.ok || !(prep.log && prep.log.naturalWidth > 300 && prep.log.naturalHeight > 300)) {
          await win.webContents.executeJavaScript(`(()=>{const o=document.getElementById('__nashir_main_img__');if(o&&o.dataset.nashirPrevStyle!==undefined){o.setAttribute('style',o.dataset.nashirPrevStyle);delete o.dataset.nashirPrevStyle;}return 1;})()`).catch(()=>{});
          products[i].image = '';
          continue;
        }

        // صوّر منطقة عنصر الصورة الرئيسية فقط (مثبّت أعلى يسار، بحجم box)
        ...
        // أعِد العنصر لوضعه الأصلي فوراً (يمنع بقاءه مثبّتاً أو تداخله مع منتج آخر)
        await win.webContents.executeJavaScript(`(()=>{const o=document.getElementById('__nashir_main_img__');if(o){if(o.dataset.nashirPrevStyle!==undefined){o.setAttribute('style',o.dataset.nashirPrevStyle);delete o.dataset.nashirPrevStyle;}o.removeAttribute('id');}return 1;})()`).catch(()=>{});

        if (result && result.data && result.data.length > 2000) {
          products[i].image = 'data:image/jpeg;base64,' + result.data;
        } else {
          products[i].image = ''; // لقطة فارغة — لا نرفقها
        }
      } catch(e) {
        try { await win.webContents.executeJavaScript(`(()=>{const o=document.getElementById('__nashir_main_img__');if(o){o.removeAttribute('id');}return 1;})()`).catch(()=>{}); } catch(e2){}
        products[i].image = '';
      }
    }

    // اكتب سجل التشخيص لملف يسهل فتحه ومشاركته (في مجلد بيانات التطبيق)
    try {
...
async function attachImagesViaCDP(win, filePaths) {
  if (!win || win.isDestroyed() || !filePaths || !filePaths.length) return false;
  const dbg = win.webContents.debugger;
  let attached = false;
  try {
    try { dbg.attach('1.3'); attached = true; } catch(e) {
      // قد يكون مرفقاً مسبقاً
      if (!/already attached/i.test(String(e))) throw e;
      attached = true;
    }
    await dbg.sendCommand('DOM.enable');

    // انتظر ظهور حقل ملفات يقبل الصور (حتى ~10 ثوانٍ)
    let backendNodeId = null, objectId = null;
    for (let attempt = 0; attempt < 20 && !backendNodeId && !objectId; attempt++) {
      const { root } = await dbg.sendCommand('DOM.getDocument', { depth: -1, pierce: true });
      // ابحث عن كل input[type=file]
      const { nodeIds } = await dbg.sendCommand('DOM.querySelectorAll', {
        nodeId: root.nodeId,
        selector: 'input[type="file"]',
      });
      if (nodeIds && nodeIds.length) {
        // فضّل الحقل الذي يقبل الصور
        for (const nid of nodeIds) {
          try {
            ...
            if (!accept || accept.includes('image') || accept.includes('*') || accept.includes('video')) {
              backendNodeId = nid;
              break;
            }
          } catch(e) {}
    ...
    // عيّن الملفات على الحقل — إكس يستقبلها كاختيار حقيقي
    await dbg.sendCommand('DOM.setFileInputFiles', {
      files: filePaths,
      nodeId: backendNodeId,
    });

    // امنح إكس وقتاً لرفع الصور ومعالجتها
...
async function openComposeWindow(content, images = []) {
  ...
  for (const src of (images || []).slice(0, 4)) {
    ...
    try {
      ...
      if (!m) continue;
      let ext = (m[1] || 'jpg').split('+')[0];
      if (ext === 'jpeg') ext = 'jpg';
      const buf = Buffer.from(m[2], 'base64');
      if (buf.length === 0 || buf.length > 6 * 1024 * 1024) continue;
      const fp = require('path').join(tmpDir, `img_${Date.now()}_${idx++}.${ext}`);
    ...
    win.webContents.on('did-finish-load', async () => {
      ...
      try {
        ...
        if ((result === 'OK' || result === 'ALREADY') && imgFiles.length > 0) {
          try { await attachImagesViaCDP(win, imgFiles); } catch(e) {}
        }
...
ipcMain.handle('install-update', () => {
  if (!autoUpdater) return;
  try {
    // أغلق كل النوافذ يدوياً أولاً كي لا يعجز المثبّت عن إغلاق التطبيق
    try { BrowserWindow.getAllWindows().forEach(w => { try { if (!w.isDestroyed()) w.destroy(); } catch(e){} }); } catch(e) {}

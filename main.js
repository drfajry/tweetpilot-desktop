...
async function openComposeWindow(content, images = []) {
  ...
  for (const src of (images || []).slice(0, 4)) {
    ...
    if (src.startsWith('http')) {
      // الصور بصيغ حديثة (AVIF/WEBP) لا يقبلها إكس — نحوّلها PNG عبر المتصفح مباشرة
      const needsConvert = /\.avif|\.webp|format=avif|format=webp|f_auto|\/avif\//i.test(src);
      let d = null;
      if (needsConvert) {
        d = await fetchImageViaBrowser(src);          // يفك ويحوّل PNG
        if (!d) d = await downloadImageAsDataUrl(src); // احتياط
      } else {
        d = await downloadImageAsDataUrl(src);
        if (!d) d = await fetchImageViaBrowser(src);   // احتياط (يحوّل ويتجاوز الحماية)
      }
      if (d) imgs.push(d);
    }
  }
...
ipcMain.handle('fetch-bestsellers', async (_, { source, query }) => {
  const STORES = {
    amazon: {
      ...
      // كل نتيجة منتج فيها data-asin، والرابط /dp/ASIN
      extract: `
        (() => {
          const out = [];
          ...
          for (const c of cards) {
            ...
            if (!asin) continue;
            const a = c.querySelector('h2 a, a.a-link-normal.s-no-outline, a.a-link-normal[href*="/dp/"]');
            ...
            if (imgEl) {
              ...
              if (ss) { const parts = ss.split(',').map(s=>s.trim().split(' ')[0]).filter(Boolean); if(parts.length) imgUrl = parts[parts.length-1]; }
              if (!imgUrl) imgUrl = imgEl.src || '';
            }
    ...
    noon: {
      ...
      extract: `
        (() => {
          // استخرج رابط صورة حقيقي من عنصر img (يتجاوز التحميل الكسول وشعار المتجر)
          function realImg(img){
            if(!img) return '';
            const cands = [];
            // srcset: خذ أعلى دقة
            const ss = img.getAttribute('srcset') || img.getAttribute('data-srcset');
            if(ss){ const parts = ss.split(',').map(s=>s.trim().split(' ')[0]).filter(Boolean); if(parts.length) cands.push(parts[parts.length-1]); }
            cands.push(img.getAttribute('data-src'));
            cands.push(img.getAttribute('data-lazy-src'));
            cands.push(img.src);
            for(const c of cands){
              if(c && /^https?:/.test(c) && !/logo|placeholder|sprite|icon|\\.svg/i.test(c)) return c;
            }
            return '';
          }
          const out = [];
          ...
          for (const a of links) {
            const href = a.href || '';
            if (!/\\/p(\\/|\\b)/.test(href)) continue;
            const clean = href.split('?')[0];
            if (seen.has(clean)) continue;
            seen.add(clean);

            // اسم المنتج: نون يضعه في aria-label أو title على الرابط (الأوثق، مثل data-asin في أمازون)
            let title = a.getAttribute('aria-label') || a.getAttribute('title') || '';
            title = title.trim();
            // احتياط: alt الصورة الوصفي
            if(!title){
              const im = a.querySelector('img');
              if(im && im.alt) title = im.alt.trim();
            }
            // رفض ما يبدو سعراً/مخزوناً لا اسماً
            if(!title || title.length < 6) continue;
            if(/^(placeholder|image|loading)$/i.test(title)) continue;
            if(/خصم|باقي|المخزون|وحدات/.test(title) && !/[A-Za-z\\u0621-\\u064A]{6,}/.test(title.replace(/[\\d%]/g,''))) continue;

            const img = a.querySelector('img');
            out.push({ url: clean, title: title.substring(0,80), price: '', image: realImg(img) });
            if (out.length >= 8) break;
          }
          return out;
        })()
      ...
      extract: `
        (() => {
          function realImg(img){
            ...
            // أحياناً الصورة كخلفية CSS
            try { const bg = getComputedStyle(img).backgroundImage; const bm = bg && bg.match(/url\\(["']?(.*?)["']?\\)/); if(bm) cands.push(bm[1]); } catch(e){}
            for(let c of cands){
              if(!c) continue;
              if(c.startsWith('//')) c = 'https:' + c;
              if(/^https?:/.test(c) && !/logo|placeholder|sprite|icon|\\.svg|blank|1x1|spacer/i.test(c)) return c;
            }
            return '';
          }
          const out = [];
          ...
          for (const a of links) {
            ...
            const imgUrl = realImg(img);
            // تجنّب تكرار نفس الصورة لمنتجات مختلفة (سبب ظهور صور متداخلة)
            if (imgUrl && seenImg.has(imgUrl)) continue;
            if (imgUrl) seenImg.add(imgUrl);
            let price = '';
            const box = a.closest('div');
            if (box) { const pm = box.textContent.match(/(?:US\\s*\\$|\\$|SAR|ريال)\\s*[\\d,\\.]+/); if (pm) price = pm[0]; }
            out.push({ url: clean, title: title.substring(0,80), price, image: imgUrl });
            if (out.length >= 8) break;
          }
          return out;
        })()
  ...
  return await new Promise((resolve) => {
    ...
    const hardTimeout = setTimeout(() => finish({ success: false, error: 'انتهت المهلة — حاول مجدداً', products: [] }), 40000);
    ...
    const tryExtract = async (attempt) => {
      ...
      try {
        ...
        if (products && products.length > 0) {
          ...
          // فلترة أخيرة + توحيد الحقول لما تتوقعه الواجهة (name)
      ...
      if (attempt < 14) setTimeout(() => tryExtract(attempt + 1), 2000);

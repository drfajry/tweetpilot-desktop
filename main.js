...
async function captureNoonImages(win, products) {
  ...
  try {
    ...
    for (let i = 0; i < products.length; i++) {
      ...
      try {
        // كبّر الصورة لحجم كبير ثابت قبل اللقطة (الصورة المصدر عالية الدقة، فقط معروضة صغيرة)
        const rect = await win.webContents.executeJavaScript(`
          (() => {
            try {
              ...
              if (!img) return null;
              // أنشئ نسخة مكبّرة من الصورة في طبقة علوية ثابتة (الصورة الأصلية عالية الدقة في nooncdn)
              let overlay = document.getElementById('__nashir_capture__');
              if (!overlay) {
                overlay = document.createElement('div');
                overlay.id = '__nashir_capture__';
                overlay.style.cssText = 'position:fixed;top:0;left:0;z-index:2147483647;background:#fff;padding:0;margin:0;';
                document.body.appendChild(overlay);
              }
              // استبدل رابط الصورة بأكبر نسخة (نون يدعم تكبير عبر حذف /45/ أو رفع الأبعاد)
              let hiRes = img.src;
              // نون: الرابط فيه مقطع مقاس مثل /45/ (مصغّر) — ارفعه لمقاس أكبر للحصول على دقة أعلى
              try { hiRes = hiRes.replace(/\\/(\\d{2,3})\\/_?\\//, '/800/_/').replace(/_\\d+x\\d+/, '_800x800'); } catch(e){}
              overlay.innerHTML = '';
              const big = document.createElement('img');
              big.crossOrigin = '';
              big.referrerPolicy = 'no-referrer';
              big.onerror = () => { big.src = img.src; }; // إن فشل المقاس الكبير، عُد للأصلي
              big.src = hiRes;
        ...
        `).catch(()=>null);
        if (!rect) continue;
        // انتظر تحميل الصورة المكبّرة
        await new Promise(r => setTimeout(r, 800));

        const result = await dbg.sendCommand('Page.captureScreenshot', {
          format: 'jpeg',
          quality: 95,
          clip: { x: 0, y: 0, width: 600, height: 600, scale: 2 }, // scale:2 = دقة مضاعفة
    ...
    // نظّف الطبقة
    try { await win.webContents.executeJavaScript(`(()=>{const o=document.getElementById('__nashir_capture__');if(o)o.remove();return 1;})()`).catch(()=>{}); } catch(e) {}
    return products;
  } catch(e) {
    return products;
  } finally {

...
ipcMain.handle('diagnose-images', async () => {
  ...
  try {
    ...
    if (noonUrl && noonUrl.startsWith('http')) {
          ...
          w.webContents.on('did-finish-load', async () => {
            try {
              ...
              const res = await w.webContents.executeJavaScript(`
                (async () => {
                  try {
                    const r = await fetch(${JSON.stringify(noonUrl)});
                    const ct = r.headers.get('content-type');
                    const cl = r.headers.get('content-length');
                    let blobInfo = null;
                    try { const b = await r.blob(); blobInfo = { type: b.type, size: b.size }; } catch(e) { blobInfo = { blobErr: String(e).slice(0,60) }; }
                    return { status: r.status, statusText: r.statusText, contentType: ct, contentLength: cl, blob: blobInfo };
                  } catch(e) { return { fetchError: String(e).slice(0,100) }; }
                })()
              `);
              done(res);
      ...
      if (ip.status === 200 && ip.blob && ip.blob.size > 0) {
        trace.q6_diagnosis = '✅ fetch داخل صفحة نون نجح (200) — المسار الصحيح: جلب الصورة داخل صفحة نون وإرسالها data-url. المشكلة Origin فقط.';
      } else if (ip.status && ip.status !== 200) {
        trace.q6_diagnosis = `⚠️ fetch داخل صفحة نون رجع ${ip.status} — ليست مشكلة Origin فقط، قد تكون حماية CDN/توقيع رابط.`;
      } else if (ip.fetchError) {
        trace.q6_diagnosis = `⚠️ fetch داخل صفحة نون فشل (${ip.fetchError}) — قد تكون حماية CDN أعمق من Origin.`;
      } else {
        trace.q6_diagnosis = 'غير حاسم — راجع IN_PAGE_FETCH';
      }
    } else {

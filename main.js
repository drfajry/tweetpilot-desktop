...
async function captureNoonImages(parentWin, products) {
  ...
  const saveShot = (b64) => {
    try {
      ...
      if (!buf || buf.length < 2000) return { url: '', size: 0 };
      const fp = pathx.join(noonImgDir, `noon_${Date.now()}_${Math.random().toString(36).slice(2,8)}.jpg`);
      fsx.writeFileSync(fp, buf);
      let size = 0; try { size = fsx.statSync(fp).size; } catch(e){}
      return { url: urlx.pathToFileURL(fp).href, size }; // file://...
    } catch(e) { return { url: '', size: 0, err: String(e).slice(0,80) }; }
  };
  ...
  let attached = false;
  const captureLog = [];
  const DEADLINE = Date.now() + 80000; // مهلة كلية صارمة
  ...
  const loadAndWait = (url, timeout) => new Promise((resolve) => {
    ...
    try { capWin.webContents.on('did-finish-load', onLoad); } catch(e){}
  ...
  try {
    ...
    for (let i = 0; i < products.length; i++) {
      const p = products[i];
      const entry = { url: p.url, captureSuccess: false, reason: '', savedFile: '', fileSize: 0, imageWidth: 0, imageHeight: 0 };
      // لا fallback لرابط Noon إطلاقاً: عند أي فشل → المنتج بلا صورة (image='') وسبب واضح في الـ Log.
      if (Date.now() > DEADLINE) { entry.reason = 'deadline_exceeded'; products[i].image = ''; captureLog.push(entry); console.log('[noon-capture]', JSON.stringify(entry)); continue; }
      if (!p.url) { entry.reason = 'no_url'; products[i].image = ''; captureLog.push(entry); console.log('[noon-capture]', JSON.stringify(entry)); continue; }
      if (!alive()) { entry.reason = 'capture_window_gone'; products[i].image = ''; captureLog.push(entry); console.log('[noon-capture]', JSON.stringify(entry)); continue; }
      ...
      try {
        ...
        entry.loaded = loaded;
        await sleep(1200);
        if (!alive()) { entry.reason = 'render_crashed_after_load'; products[i].image = ''; captureLog.push(entry); console.log('[noon-capture]', JSON.stringify(entry)); continue; }

        // (2)+(3) انتظر الصورة الرئيسية الأصلية (complete && naturalWidth>500 && naturalHeight>500) — مهلة 9s
        const prep = await withTimeout(capWin.webContents.executeJavaScript(`
          (async () => {
            ...
            try {
              ...
              if (!best) return { ok:false, reason:'no_image_ge_500', log: lastLog };
              ...
              await sleep(160);
              return { ok:true, box: box, w: best.naturalWidth, h: best.naturalHeight, log: lastLog };
            } catch(e) { return { ok:false, reason:'page_js_exception', log:{ err:String(e).slice(0,120) } }; }
          })()
        `), 9000, { ok:false, reason:'image_wait_timeout' });

        // سجّل أبعاد الصورة المكتشفة (إن وُجدت)
        if (prep && prep.w) { entry.imageWidth = prep.w; entry.imageHeight = prep.h; }
        else if (prep && prep.log && prep.log.naturalWidth) { entry.imageWidth = prep.log.naturalWidth; entry.imageHeight = prep.log.naturalHeight; }

        if (prep && prep.ok && prep.box && attached && alive()) {
          // (4) صوّر الصورة الرئيسية فقط — scale 2
          ...
          if (shot && shot.data && shot.data.length > 2000) {
            const saved = saveShot(shot.data); // احفظ كملف
            if (saved.url) { products[i].image = saved.url; entry.captureSuccess = true; entry.reason = 'ok'; entry.savedFile = saved.url; entry.fileSize = saved.size; }
            else { products[i].image = ''; entry.reason = 'file_write_failed' + (saved.err ? (':' + saved.err) : ''); }
          } else {
            products[i].image = ''; entry.reason = 'cdp_screenshot_failed';
          }
        } else {
          // لم نجد صورة ≥500 في صفحة المنتج → بلا صورة (لا رابط Noon)
          products[i].image = '';
          entry.reason = (prep && prep.reason) || 'no_image_ge_500';
        }
      } catch(e) {
        products[i].image = '';
        entry.reason = 'exception:' + String(e).slice(0, 100);
      }
      captureLog.push(entry);
      console.log('[noon-capture]', JSON.stringify({
        url: entry.url, captureSuccess: entry.captureSuccess, reason: entry.reason,
        savedFile: entry.savedFile, fileSize: entry.fileSize,
        imageWidth: entry.imageWidth, imageHeight: entry.imageHeight
      }));
    }

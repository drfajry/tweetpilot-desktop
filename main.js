    ...
    for (let i = 0; i < products.length; i++) {
      ...
      try {
        ...
        if (prep && prep.ok && prep.box && alive()) {
          ...
          if (buf) {
            const fileUrl = saveBuf(buf);
            let fileSize = 0; try { if (fileUrl) fileSize = require('fs').statSync(require('url').fileURLToPath(fileUrl)).size; } catch(e){}
            console.log('[capture] saveBuf', JSON.stringify({ method: capMethod, bufLength: buf.length, fileUrl: fileUrl || '(empty)', fileSize }));
            ...
            products[i].image = p.image || ''; entry.captured = false;
          }
          // (6) قيمة الصورة النهائية بعد الاستبدال
  ...
  try {
    ...
    console.log = (...args) => {
      ...
      try {
        ...
        if (/^\[(PRE_PUBLISH_IMAGE|x-image|x-upload|noon-capture|capture|updater)\]/.test(line)) {

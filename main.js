    ...
    noon: {
      ...
      extract: `
        (() => {
          ...
          // نون: روابط المنتجات تحتوي /p/ أو معرّف يبدأ بـ N
          ...
          for (const a of links) {
            ...
            // ابحث عن عنوان حقيقي: جرّب النص داخل الرابط، ثم الحاوية الأكبر، مع رفض alt الوهمي (placeholder)
            const container = a.closest('div[class*="product"], div[class*="Product"], li, article') || a.parentElement?.parentElement || a;
            let title = '';
            // 1) نصوص العناصر داخل الرابط/الحاوية التي تبدو كعنوان (طويلة نسبياً)
            const textNodes = container.querySelectorAll('*');
            let best = '';
            for (const el of textNodes) {
              const t = (el.textContent || '').trim();
              if (t.length > best.length && t.length >= 8 && t.length <= 150 && !/^[\\d.,]+$/.test(t) && !/ريال|SAR/i.test(t)) {
                best = t;
              }
            }
            title = best;
            // 2) احتياط: alt الصورة إن لم يكن وهمياً
            if (!title) {
              const img = a.querySelector('img') || container.querySelector('img');
              if (img && img.alt && !BAD.test(img.alt.trim())) title = img.alt.trim();
            }

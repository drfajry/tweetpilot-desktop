...
ipcMain.handle('diagnose-images', async () => {
  ...
  try {
    ...
    if (noonUrl && noonUrl.startsWith('http')) {
      ...
      // هل يُفك محلياً؟ (نستخدم nativeImage المدمج في Electron بدل Sharp/Jimp)
      try {
        const { nativeImage } = require('electron');
        const dataForDecode = direct || (trace.q1b_browserFetch === 'نجح' ? await fetchImageViaBrowser(noonUrl) : null);
        if (dataForDecode) {
          ...
          trace.q4_localDecode = 'لا توجد بيانات لفكّها';
        }
      } catch(e) { trace.q4_localDecode = 'خطأ: ' + String(e).slice(0,50); }

      // أين المشكلة بالضبط؟
      if (!noonUrl) trace.q6_diagnosis = 'المشكلة في استخراج الرابط';
      else if (trace.q1_directDownload === 'فشل — جرّبنا المتصفح' && trace.q1b_browserFetch === 'فشل أيضاً') trace.q6_diagnosis = 'المشكلة في تنزيل الصورة (نون يحجب التحميل من خارج صفحته)';
      else if (/فشل/.test(trace.q4_localDecode||'')) trace.q6_diagnosis = 'المشكلة في فكّ الصورة';
      else trace.q6_diagnosis = 'التنزيل والفكّ نجحا — المشكلة في الإرفاق (CDP) أو الصيغة';
    } else {

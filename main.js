...
async function captureNoonImages(parentWin, products) {
  ...
  try {
    ...
    for (let i = 0; i < products.length; i++) {
      ...
      if (!alive()) { entry.skipped = 'win_gone'; products[i].image = p.image || ''; captureLog.push(entry); continue; }
      ...
      try {
        ...
        entry.loaded = loaded;
        await sleep(1200);
        if (!alive()) { entry.crashed = true; products[i].image = p.image || ''; captureLog.push(entry); continue; }
        ...
        if (prep && prep.ok && prep.box && attached && alive()) {
          // (4)+(5) صوّر الصورة الرئيسية فقط — scale 2 (الحجم لم يعد يهم: نمرّر مسار ملف لا data URI)
          ...
          if (shot && shot.data && shot.data.length > 2000) {
            const fileUrl = saveShot(shot.data); // احفظ كملف ومرّر مساره
            if (fileUrl) { products[i].image = fileUrl; entry.captured = 'product_page'; entry.savedFile = fileUrl; }
            else { products[i].image = p.image || ''; entry.captured = false; }
          } else { products[i].image = p.image || ''; entry.captured = false; }
        } else {
          // ── خطة بديلة (فقط عند تعذّر صفحة المنتج): رفع دقة currentSrc/thumbnail ──
          entry.stage = 'fallback_currentSrc';
          const fb = (attached && alive()) ? await captureFallbackHiRes(capWin, dbg, p.image, sleep) : { ok:false, log:{} };
          Object.assign(entry, fb.log || {});
          if (fb.ok && fb.b64) {
            const fileUrl = saveShot(fb.b64);
            if (fileUrl) { products[i].image = fileUrl; entry.captured = 'fallback'; entry.savedFile = fileUrl; }
            else { products[i].image = p.image || ''; entry.captured = false; }
          }
...
async function openComposeWindow(content, images = []) {
  ...
  for (const src of (images || []).slice(0, 4)) {
    ...
    if (src.startsWith('file://')) {
      // صورة نون محفوظة كملف — نقرأها من القرص مباشرة (لا data URI عبر IPC)
      try { buf = require('fs').readFileSync(require('url').fileURLToPath(src)); } catch(e) { continue; }
      ...
      if ((!d || !d.startsWith('data:image')) && httpSrc !== src) {
        // لو فشل الرابط عالي الدقة، اسقط للأصلي

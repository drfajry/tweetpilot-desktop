...
ipcMain.handle('fetch-bestsellers', async (_, { source, query }) => {
  const STORES = {
    ...
    noon: {
      ...
      extract: `
        (() => {
          const out = [];
          // بطاقات نون: روابط تنتهي بـ /p/
          const links = document.querySelectorAll('a[href*="/p/"], a[href*="/N"][href*="/p"]');
          const seen = new Set();
          for (const a of links) {
            let href = a.href;
            if (!href || !/\\/p\\b/.test(href) || seen.has(href)) continue;
            seen.add(href);
            ...
            if (box) { const pm = box.textContent.match(/[\\d,\\.]+\\s*(?:ريال|SAR|SR)/); if (pm) price = pm[0]; }
            out.push({ url: href.split('?')[0], title: title.substring(0,80), price, image: img ? img.src : '' });
            if (out.length >= 8) break;
          }
          return out;
        })()
  ...
  return await new Promise((resolve) => {
    ...
    // مهلة قصوى 25 ثانية
    const hardTimeout = setTimeout(() => finish({ success: false, error: 'انتهت المهلة — حاول مجدداً', products: [] }), 25000);
    ...
    const tryExtract = async (attempt) => {
      ...
      // أعد المحاولة حتى 6 مرات (النتائج تُحمّل ديناميكياً)
      if (attempt < 6) setTimeout(() => tryExtract(attempt + 1), 1800);
      else { clearTimeout(hardTimeout); finish({ success: false, error: 'لم نجد منتجات لهذا البحث — جرّب كلمة أخرى', products: [] }); }
    };

    win.webContents.on('did-finish-load', () => {
      // امنح الصفحة وقتاً لتحميل النتائج الديناميكية ثم اقرأ
      setTimeout(() => tryExtract(0), 2000);
    });

...
async function openComposeWindow(content, images = []) {
  ...
  for (const src of (images || []).slice(0, 4)) {
    ...
    // (1) نوع الصورة + المصدر
    const srcType = src.startsWith('file://') ? 'LocalFile' : src.startsWith('data:video') ? 'Video' : src.startsWith('data:image') ? 'Base64' : src.startsWith('http') ? 'URL' : (src.startsWith('blob:') ? 'Blob' : 'Unknown');
    console.log('[x-image] processing', JSON.stringify({ type: srcType, source: src.slice(0, 90) }));
    // ── فيديو: يُحفظ كما هو (دون تطبيع/تحويل) ويُرفق مباشرة ──
    if (src.startsWith('data:video')) {
      try {
        const mv = src.match(/^data:(video\/[a-z0-9.+-]+);base64,(.+)$/i);
        if (mv) {
          let ext = mv[1].split('/')[1].toLowerCase();
          if (ext === 'quicktime') ext = 'mov';
          if (!/^(mp4|mov|webm|m4v)$/.test(ext)) ext = 'mp4';
          const vbuf = Buffer.from(mv[2], 'base64');
          if (vbuf.length > 0 && vbuf.length <= 512 * 1024 * 1024) {
            const vp = require('path').join(tmpDir, `vid_${Date.now()}_${idx++}.${ext}`);
            require('fs').writeFileSync(vp, vbuf);
            imgFiles.push(vp);
            console.log('[x-video] saved', JSON.stringify({ path: vp, size: vbuf.length, ext }));
          } else {
            console.log('[x-video] skipped — size', vbuf.length);
          }
        }
      } catch(e) { console.log('[x-video] error', String(e).slice(0,100)); }
      continue;
    }
...
ipcMain.handle('fetch-trends', async (_, { region, platform }) => {
  ...
  if (platform === 'google') {
    try {
      const reg = (region || 'sa').toLowerCase();
      const res = await fetch(`${TRENDS_SERVER}/api/trends/tiktok?region=${reg}`); // ترندات حسب المنطقة
...
// ════════════════ رمز إحالة (سوّق ناشر واربح) — يُرسل مرة واحدة عند أول تشغيل ════════════════
function affiliateFlagPath() {
  try { return require('path').join(app.getPath('userData'), '.affiliate-sent'); } catch (e) { return null; }
}
ipcMain.handle('submit-affiliate-ref', async (_, refCode) => {
  const code = String(refCode || '').trim().toUpperCase();
  if (!code) return { success: false, error: 'أدخل رمز الإحالة' };
  const flagFile = affiliateFlagPath();
  try {
    const fsx = require('fs');
    if (flagFile && fsx.existsSync(flagFile)) return { success: false, error: 'سبق إرسال رمز إحالة من هذا الجهاز' };
  } catch (e) {}

  return await new Promise((resolve) => {
    const body = JSON.stringify({ refCode: code, deviceId: getDeviceId() });
    const options = {
      hostname: 'nashir-affiliate.onrender.com', path: '/api/app-referral', method: 'POST',
    ...
    const req = https.request(options, (r) => {
      ...
      r.on('end', () => {
        try {
          ...
          if (json.success) { try { require('fs').writeFileSync(flagFile, new Date().toISOString()); } catch (e) {} }
          resolve(json);
        } catch (e) { resolve({ success: false, error: 'استجابة غير صالحة من الخادم' }); }
      });
    });
    req.on('error', e => resolve({ success: false, error: 'تعذّر الاتصال — تحقق من الإنترنت' }));
